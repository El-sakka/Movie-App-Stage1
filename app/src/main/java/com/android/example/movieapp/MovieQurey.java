...
public class MovieQurey {

    private static final String LOG_TAG = MovieQurey.class.getSimpleName();
    ...
    private static List<MovieApp> extractMovieData(String movieJson) {
        if (TextUtils.isEmpty(movieJson)) {
            return null;
        }
        List<MovieApp> movies = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < resultArray.length(); i++) {
                ...
                //Log.v("posterImage",posterPath);

                MovieApp movie = new MovieApp(title, posterPath, releaseDate, voteAverage, description);
                movies.add(movie);
            }
    ...
    public static List<MovieApp> fetchMovieAppData(String requestUrl) {
        URL url = makeUrl(requestUrl);
        String jsonResponse = null;
        try {
            jsonResponse = makeHttpRequest(url);

            Log.e("Mahmoud","HHHHHHH"+jsonResponse);

        } catch (IOException e) {
            Log.e(LOG_TAG, "Throw an Exception in fetchMovieAppData", e);
        }
        List<MovieApp> movies = extractMovieData(jsonResponse);
        return movies;
    }
